...
async def track_aht(
    data: AHTTrackRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await wfm_metrics_service.track_aht(
        data.call_id, data.agent_id, data.duration_seconds, tenant_id
    )
    if not result:
...
async def track_fcr(
    data: FCRTrackRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await wfm_metrics_service.track_fcr(
        data.call_id, data.customer_id, data.resolved, tenant_id, data.follow_up_call_id
    )
    if not result:
...
async def track_csat(
    data: CSATTrackRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await wfm_metrics_service.track_csat(
        data.call_id, data.customer_id, data.rating, tenant_id
    )
    if not result:
...
async def track_nps(
    data: NPSTrackRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await wfm_metrics_service.track_nps(
        data.call_id, data.customer_id, data.score, tenant_id
    )
    if not result:
...
async def create_course(
    data: CourseCreateRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await training_service.create_course(
        tenant_id, data.title, data.description, data.modules, data.duration_hours
    )
    if not result:
...
async def enroll_agent(
    data: EnrollRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await training_service.enroll_agent(
        tenant_id, data.agent_id, data.course_id
    )
    if not result:
...
async def track_progress(
    data: ProgressRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await training_service.track_progress(
        data.enrollment_id, data.module_id, data.status
    )
    if not result:
...
async def get_certifications(
    agent_id: str, tenant_id: str = Depends(verify_tenant_access)
):
...
async def create_coaching(
    data: CoachingCreateRequest, tenant_id: str = Depends(verify_tenant_access)
):
    result = await training_service.create_coaching_session(
        tenant_id, data.agent_id, data.coach_id, data.focus_area, data.notes
    )
    if not result:
