...
MIN_AUDIO_SIZE_BYTES = (
    32 * 1024
)  # ~2s at 16kHz mono 16-bit; reject trivially short clips
...
async def clone_voice(
    ...
    try:
        ...
        temp_path = os.path.join(
            VOICE_CLONES_DIR, f"{voice_id}_{uuid.uuid4().hex}_temp"
        )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    chatterbox_id = (
                        result.get("id") or result.get("voice_id") or voice_id
                    )
                    voice_profile["chatterbox_voice_id"] = chatterbox_id
                    voice_profile.update(
                        {k: v for k, v in result.items() if k not in ("id", "voice_id")}
                    )
                    voice_profile["engine"] = "chatterbox"
                else:
                    body = await resp.text()
                    logger.warning(
                        "chatterbox_clone_bad_status",
                        status=resp.status,
                        body=body[:200],
                    )
