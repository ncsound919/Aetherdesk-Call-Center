...
async def create_dr_test_db(tenant_id, test_type, status, result, duration_seconds):
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM dr_tests WHERE id = ?", (test_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
    ...
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM dr_tests WHERE tenant_id = $1 ORDER BY tested_at DESC",
                tenant_id,
            )
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM dr_tests WHERE tenant_id = ? ORDER BY tested_at DESC",
                (tenant_id,),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
    ...
    if USE_POSTGRES:
        ...
        if pool:
            row = await pool.fetchrow(
                "SELECT * FROM rate_limit_configs WHERE tenant_id = $1 AND route_key = $2",
                tenant_id,
                route_key,
            )
            return dict(row) if row else None
        ...
        try:
            row = conn.execute(
                "SELECT * FROM rate_limit_configs WHERE tenant_id = ? AND route_key = ?",
                (tenant_id, route_key),
            ).fetchone()
            return dict(row) if row else None
        finally:
...
async def set_rate_limit_config_db(tenant_id, route_key, max_requests, window_seconds):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM rate_limit_configs WHERE id = $1", config_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM rate_limit_configs WHERE id = ?", (row_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def list_rate_limit_configs_db(tenant_id=None):
    if USE_POSTGRES:
        ...
        if pool:
            if tenant_id:
                rows = await pool.fetch(
                    "SELECT * FROM rate_limit_configs WHERE tenant_id = $1 ORDER BY route_key",
                    tenant_id,
                )
            else:
                rows = await pool.fetch(
                    "SELECT * FROM rate_limit_configs ORDER BY tenant_id, route_key"
                )
            return [dict(r) for r in rows]
    else:
        conn = _get_sqlite_conn()
        try:
            if tenant_id:
                rows = conn.execute(
                    "SELECT * FROM rate_limit_configs WHERE tenant_id = ? ORDER BY route_key",
                    (tenant_id,),
                ).fetchall()
            else:
                rows = conn.execute(
...
async def log_circuit_breaker_event_db(
    breaker_name, from_state, to_state, failure_count
):
...
async def list_circuit_breaker_events_db(breaker_name=None, limit=100):
    if USE_POSTGRES:
        ...
        if pool:
            if breaker_name:
                rows = await pool.fetch(
                    "SELECT * FROM circuit_breaker_events WHERE breaker_name = $1 ORDER BY timestamp DESC LIMIT $2",
                    breaker_name,
                    limit,
                )
            else:
                rows = await pool.fetch(
                    "SELECT * FROM circuit_breaker_events ORDER BY timestamp DESC LIMIT $1",
                    limit,
                )
            return [dict(r) for r in rows]
    else:
        ...
        try:
            if breaker_name:
                rows = conn.execute(
                    "SELECT * FROM circuit_breaker_events WHERE breaker_name = ? ORDER BY timestamp DESC LIMIT ?",
                    (breaker_name, limit),
                ).fetchall()
            else:
                rows = conn.execute(
