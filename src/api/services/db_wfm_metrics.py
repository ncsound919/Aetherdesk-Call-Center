...
async def create_aht_db(tenant_id, agent_id, call_id, duration_seconds):
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wfm_aht WHERE id = ?", (record_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def create_fcr_db(
    tenant_id, customer_id, call_id, resolved, follow_up_call_id=None
):
    record_id = str(uuid.uuid4())
    if USE_POSTGRES:
        pool = await get_pg_pool()
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wfm_fcr WHERE id = ?", (record_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def create_csat_db(tenant_id, customer_id, call_id, rating):
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wfm_csat WHERE id = ?", (record_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def create_nps_db(tenant_id, customer_id, call_id, score):
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM wfm_nps WHERE id = ?", (record_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def get_fcr_stats_db(tenant_id, period="7d"):
    ...
    return {
        "fcr_rate": round(resolved / total * 100, 2),
        "resolved": resolved,
        "total": total,
    }
...
async def list_aht_db(tenant_id, limit=50):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM wfm_aht WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2",
                tenant_id,
                limit,
            )
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM wfm_aht WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?",
                (tenant_id, limit),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def list_fcr_db(tenant_id, limit=50):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM wfm_fcr WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2",
                tenant_id,
                limit,
            )
            return [dict(r) for r in rows]
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM wfm_fcr WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?",
                (tenant_id, limit),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def list_csat_db(tenant_id, limit=50):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM wfm_csat WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2",
                tenant_id,
                limit,
            )
            return [dict(r) for r in rows]
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM wfm_csat WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?",
                (tenant_id, limit),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
