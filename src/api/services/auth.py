...
class TokenStore:
    async def create_token(self, user_id: str, metadata: dict = None) -> str:
        ...
        if redis_client:
            await redis_client.setex(
                f"ws_token:{token}", TOKEN_EXPIRY_SECONDS, json.dumps(token_data)
            )
        else:
    ...
    async def validate_token(self, token: str) -> dict | None:
        ...
        if redis_client:
            ...
            if (
                token_data
                and time.time() - token_data["created_at"] <= TOKEN_EXPIRY_SECONDS
            ):
                return token_data
    ...
    def cleanup_expired(self):
        if hasattr(self, "_fallback_tokens"):
            ...
            expired = [
                t
                for t, data in self._fallback_tokens.items()
...
def generate_access_token(
    data: dict, expires_delta_seconds: int = TOKEN_EXPIRY_SECONDS
) -> str:
...
async def verify_access_token(token: str) -> dict | None:
    ...
    from api.services.jwt_utils import verify_access_token as _verify_rs256_token
...
async def verify_api_key(x_api_key: str = Header(default=None)) -> str:
    ...
    from api.services.database import get_tenant_by_api_key
...
async def verify_tenant_access(
    ...
    """
    if x_api_key == INTERNAL_API_KEY:
        return tenant_id  # Internal key always allowed

    try:
        from api.services.database import verify_tenant_api_key
...
# MFA integration
async def is_mfa_required(user_id: str) -> bool:
...
async def create_mfa_session_token(
    ...
    """Create a temporary session token for MFA step (valid for 5 minutes)."""
    from datetime import timedelta
    ...
    return _create_rs256_token(data, timedelta(minutes=5))


async def create_full_token(user_id: str, tenant_id: str, email: str, role: str) -> str:
    """Create full JWT after MFA verification."""
    from datetime import timedelta
    ...
    return _create_rs256_token(data, timedelta(hours=24))
