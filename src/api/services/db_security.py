    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM pen_test_scans WHERE id = $1", scan_id
            )
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM pen_test_scans WHERE id = ?", (scan_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
    ...
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM pen_test_scans WHERE tenant_id = $1 ORDER BY started_at DESC",
                tenant_id,
            )
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM pen_test_scans WHERE tenant_id = ? ORDER BY started_at DESC",
                (tenant_id,),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def create_waf_event_db(tenant_id, rule_id, action, source_ip, request_path):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM waf_events WHERE id = $1", event_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM waf_events WHERE id = ?", (event_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def list_waf_events_db(tenant_id, limit=100):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM waf_events WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2",
                tenant_id,
                limit,
            )
            return [dict(r) for r in rows]
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM waf_events WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?",
                (tenant_id, limit),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def set_data_classification_db(
    tenant_id, schema_name, table_name, column_name, sensitivity, description=None
):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM data_classification WHERE id = $1", class_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM data_classification WHERE id = ?", (row_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def get_data_classification_db(tenant_id):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM data_classification WHERE tenant_id = $1 ORDER BY schema_name, table_name, column_name",
                tenant_id,
            )
            return [dict(r) for r in rows]
    else:
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM data_classification WHERE tenant_id = ? ORDER BY schema_name, table_name, column_name",
                (tenant_id,),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def create_rbac_audit_result_db(
    tenant_id, role, resource, action, expected, actual, passed
):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM rbac_audit_results WHERE id = $1", audit_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM rbac_audit_results WHERE id = ?", (audit_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def list_rbac_audit_results_db(tenant_id):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM rbac_audit_results WHERE tenant_id = $1 ORDER BY tested_at DESC",
                tenant_id,
            )
            return [dict(r) for r in rows]
    else:
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM rbac_audit_results WHERE tenant_id = ? ORDER BY tested_at DESC",
                (tenant_id,),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
