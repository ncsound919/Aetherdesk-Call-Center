...
async def upsert_customer_profile_db(tenant_id, customer_id, **kwargs):
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM customer_profiles WHERE id = ?", (customer_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
    ...
    if USE_POSTGRES:
        ...
        try:
            rows = conn.execute(
                ...
                LIMIT 50
            """,
                (tenant_id, like_val, like_val, like_val),
            ).fetchall()
            results = [dict(r) for r in rows]
        finally:
            conn.close()
...
async def update_customer_tags_db(customer_id, tags):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            await pool.execute(
                "UPDATE customer_profiles SET tags_json = $1::jsonb, updated_at = NOW() WHERE id = $2",
                tags_json,
                customer_id,
            )
    else:
        ...
        try:
            now = datetime.now(UTC).isoformat()
            conn.execute(
                "UPDATE customer_profiles SET tags_json = ?, updated_at = ? WHERE id = ?",
                (tags_json, now, customer_id),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    if USE_POSTGRES:
        ...
        if pool:
            row = await pool.fetchval(
                "SELECT tags_json FROM customer_profiles WHERE id = $1", customer_id
            )
            if row:
        ...
        try:
            row = conn.execute(
                "SELECT tags_json FROM customer_profiles WHERE id = ?", (customer_id,)
            ).fetchone()
            if row:
...
async def create_customer_interaction_db(
    tenant_id,
    customer_id,
    interaction_type,
    channel="voice",
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM customer_interactions WHERE id = $1", interaction_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM customer_interactions WHERE id = ?", (interaction_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def create_segment_db(tenant_id, name, criteria_json):
    ...
    criteria_str = (
        json.dumps(criteria_json) if isinstance(criteria_json, dict) else criteria_json
    )
    if USE_POSTGRES:
        pool = await get_pg_pool()
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM customer_segments WHERE id = $1", segment_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM customer_segments WHERE id = ?", (segment_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def list_segments_db(tenant_id):
    if USE_POSTGRES:
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM customer_segments WHERE tenant_id = ? ORDER BY created_at DESC",
                (tenant_id,),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def update_segment_member_count_db(segment_id, count):
    if USE_POSTGRES:
        ...
        if pool:
            await pool.execute(
                "UPDATE customer_segments SET member_count = $1 WHERE id = $2",
                count,
                segment_id,
            )
    else:
        ...
        try:
            conn.execute(
                "UPDATE customer_segments SET member_count = ? WHERE id = ?",
                (count, segment_id),
            )
            conn.commit()
        finally:
