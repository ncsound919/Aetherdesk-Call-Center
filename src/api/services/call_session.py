...
class VoiceSession:
    ...
    async def process_audio(self, audio_chunk: bytes) -> str | None:
        ...
        async with self._buffer_lock:
            ...
            should_transcribe = (
                (
                    self.silence_duration >= self.MAX_SILENCE_FRAMES
                    and len(self.audio_buffer) > 3200
                )
                or (
                    len(self.audio_buffer) >= self.MAX_BUFFER_SIZE // 10
                )  # ~1MB fallback
            )
    ...
    async def speak_stream(
        self, text: str, sentiment: str = "neutral", latency_ms: float = 0.0
    ):
