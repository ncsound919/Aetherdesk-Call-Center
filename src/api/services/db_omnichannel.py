    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM sms_templates WHERE id = $1", template_id
            )
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM sms_templates WHERE id = ?", (template_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def create_chat_session_db(
    tenant_id, visitor_id, visitor_name=None, visitor_email=None
):
...
async def add_chat_message_db(session_id, sender_type, content, sender_name=None):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM chat_messages WHERE id = $1", msg_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
...
async def update_chat_session_db(session_id, **kwargs):
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            params.append(session_id)
            conn.execute(
                f"UPDATE chat_sessions SET {', '.join(set_parts)} WHERE id = ?", params
            )
            conn.commit()
            row = conn.execute(
                "SELECT * FROM chat_sessions WHERE id = ?", (session_id,)
