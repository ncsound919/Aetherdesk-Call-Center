...
async def create_failover_test_db(
    tenant_id, service, result, duration_seconds, tested_by
):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM failover_tests WHERE id = $1", test_id
            )
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM failover_tests WHERE id = ?", (test_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
    ...
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM failover_tests WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2",
                tenant_id,
                limit,
            )
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM failover_tests WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?",
                (tenant_id, limit),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def create_chaos_experiment_db(tenant_id, target, fault_type, duration_seconds):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM chaos_experiments WHERE id = $1", exp_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM chaos_experiments WHERE id = ?", (exp_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def update_chaos_experiment_db(exp_id, status, result):
    ...
    if USE_POSTGRES:
        ...
        try:
            conn.execute(
                "UPDATE chaos_experiments SET status = ?, result_json = ? WHERE id = ?",
...
async def list_chaos_experiments_db(tenant_id, limit=50):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM chaos_experiments WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2",
                tenant_id,
                limit,
            )
            return [dict(r) for r in rows]
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM chaos_experiments WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?",
                (tenant_id, limit),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def create_contract_db(tenant_id, vendor, terms, renewal_date, cost=None):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM vendor_contracts WHERE id = $1", contract_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM vendor_contracts WHERE id = ?", (contract_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def list_contracts_db(tenant_id):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM vendor_contracts WHERE tenant_id = $1 ORDER BY renewal_date ASC",
                tenant_id,
            )
            return [dict(r) for r in rows]
    else:
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM vendor_contracts WHERE tenant_id = ? ORDER BY renewal_date ASC",
                (tenant_id,),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
    ...
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                ...
                AND renewal_date BETWEEN NOW() AND NOW() + INTERVAL '{days_ahead} days'
        ...
        try:
            rows = conn.execute(
                ...
                AND renewal_date BETWEEN datetime('now') AND datetime('now', '+{days_ahead} days')
                ORDER BY renewal_date ASC
            """,
                (tenant_id,),
            ).fetchall()
...
async def create_backup_channel_db(tenant_id, channel_type, config):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM backup_channels WHERE id = $1", channel_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM backup_channels WHERE id = ?", (channel_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def list_backup_channels_db(tenant_id):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM backup_channels WHERE tenant_id = $1 ORDER BY created_at DESC",
                tenant_id,
            )
            return [dict(r) for r in rows]
    else:
        ...
        try:
            rows = conn.execute(
                "SELECT * FROM backup_channels WHERE tenant_id = ? ORDER BY created_at DESC",
                (tenant_id,),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
...
async def update_backup_channel_test_db(channel_id, status):
    if USE_POSTGRES:
        ...
        if pool:
            await pool.execute(
                "UPDATE backup_channels SET last_test_at = NOW(), status = $1 WHERE id = $2",
                status,
                channel_id,
            )
    else:
        ...
        try:
            now = datetime.now(UTC).isoformat()
            conn.execute(
                "UPDATE backup_channels SET last_test_at = ?, status = ? WHERE id = ?",
                (now, status, channel_id),
            )
            conn.commit()
        finally:
            conn.close()
