...
async def get_api_key_by_prefix_db(key_prefix):
    if USE_POSTGRES:
        ...
        if pool:
            row = await pool.fetchrow(
                "SELECT * FROM api_keys WHERE key_prefix = $1", key_prefix
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
...
async def register_webhook_db(tenant_id, url, events, secret):
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM webhook_configs WHERE id = ?", (cur.lastrowid,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def unregister_webhook_db(tenant_id, webhook_id):
    if USE_POSTGRES:
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "DELETE FROM webhook_configs WHERE id = ? AND tenant_id = ?",
...
async def list_webhooks_db(tenant_id):
    if USE_POSTGRES:
        ...
        if pool:
            rows = await pool.fetch(
                "SELECT * FROM webhook_configs WHERE tenant_id = $1 ORDER BY created_at DESC",
                tenant_id,
            )
            return [dict(r) for r in rows]
    else:
...
async def create_webhook_delivery_log_db(
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM webhook_delivery_logs WHERE id = ?", (cur.lastrowid,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def update_webhook_delivery_log_db(
    log_id,
    status,
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            await pool.execute(
                f"UPDATE webhook_delivery_logs SET {', '.join(set_parts)} WHERE id = ${idx}",
                *params,
            )
    else:
        conn = _get_sqlite_conn()
        try:
            ...
            params.append(log_id)
            conn.execute(
                f"UPDATE webhook_delivery_logs SET {', '.join(set_parts)} WHERE id = ?",
                params,
            )
            conn.commit()
        finally:
...
async def get_webhook_delivery_log_by_id_db(log_id):
    if USE_POSTGRES:
        ...
        if pool:
            row = await pool.fetchrow(
                "SELECT * FROM webhook_delivery_logs WHERE id = $1", log_id
            )
            return dict(row) if row else None
    else:
        conn = _get_sqlite_conn()
        try:
            row = conn.execute(
                "SELECT * FROM webhook_delivery_logs WHERE id = ?", (log_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
