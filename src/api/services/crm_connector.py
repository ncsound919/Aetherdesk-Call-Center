...
class SalesforceConnector(CRMConnector):
    ...
    async def create_contact(self, data: dict) -> dict:
        ...
        return _std_response(
            False, self.provider, error="Failed to create contact in Salesforce"
        )
    ...
    async def get_contact(self, contact_id: str) -> dict:
        ...
        return _std_response(
            False, self.provider, error=f"Contact {contact_id} not found"
        )

    async def update_contact(self, contact_id: str, data: dict) -> dict:
        logger.info("salesforce_update_contact", contact_id=contact_id)
        result = await self._request(
            "PATCH", f"/sobjects/Contact/{contact_id}", json=data
        )
        if result is not None or True:
            return _std_response(True, self.provider, {"id": contact_id, **data})
        return _std_response(
            False, self.provider, error=f"Failed to update contact {contact_id}"
        )

    async def search_contacts(self, query: str) -> dict:
        logger.info("salesforce_search_contacts", query=query)
        result = await self._request(
            "GET", "/parameterizedSearch/", params={"q": query, "sobject": "Contact"}
        )
        if result:
            return _std_response(
                True,
    ...
    async def get_health(self) -> dict:
        ...
        if result:
            return _std_response(
                True,
                self.provider,
                {"status": "healthy", "details": "Salesforce API reachable"},
            )
    ...
    async def sync_contacts(self) -> dict:
        ...
        result = await self._request(
            "GET",
            "/query/",
            params={"q": "SELECT Id, Name, Email FROM Contact LIMIT 200"},
        )
        if result:
...
class HubSpotConnector(CRMConnector):
    ...
    async def create_contact(self, data: dict) -> dict:
        ...
        result = await self._request(
            "POST", "/crm/v3/objects/contacts", json={"properties": dict(data.items())}
        )
        if result:
            return _std_response(True, self.provider, {"id": result.get("id"), **data})
        return _std_response(
            False, self.provider, error="Failed to create contact in HubSpot"
        )
    ...
    async def update_contact(self, contact_id: str, data: dict) -> dict:
        ...
        result = await self._request(
            "PATCH", f"/crm/v3/objects/contacts/{contact_id}", json={"properties": data}
        )
    ...
    async def search_contacts(self, query: str) -> dict:
        ...
        result = await self._request(
            "POST", "/crm/v3/objects/contacts/search", json={"query": query}
        )
        if result:
            results = result.get("results", [])
            return _std_response(
                True, self.provider, {"contacts": results, "total": len(results)}
            )
        return _std_response(True, self.provider, {"contacts": [], "total": 0})
    ...
    async def get_health(self) -> dict:
        ...
        try:
            resp = await self._client.get(
                "/crm/v3/objects/contacts", params={"limit": 1}
            )
            if resp.status_code == 200:
                return _std_response(
                    True,
                    self.provider,
                    {"status": "healthy", "details": "HubSpot API reachable"},
                )
            return _std_response(
                False, self.provider, error=f"HubSpot returned {resp.status_code}"
            )
        except httpx.RequestError as e:
    ...
    async def sync_contacts(self) -> dict:
        ...
        result = await self._request(
            "GET", "/crm/v3/objects/contacts", params={"limit": 200}
        )
        if result:
            results = result.get("results", [])
            return _std_response(
...
class CRMConnectorFactory:
    ...
    async def from_tenant(cls, tenant_id: str, provider: str) -> CRMConnector:
        ...
        raise ValueError(
            f"No CRM config found for provider {provider} and tenant {tenant_id}"
        )
