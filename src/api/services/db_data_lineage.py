...
async def create_lineage_entry_db(
    tenant_id,
    source_table,
    source_id,
    target_table,
    ...
    if USE_POSTGRES:
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM data_lineage WHERE id = ?", (entry_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
    ...
    if USE_POSTGRES:
        ...
        try:
            rows = conn.execute(
                ...
                ORDER BY created_at DESC
            """,
                (tenant_id, table, record_id, table, record_id),
            ).fetchall()
            return [dict(r) for r in rows]
        finally:
            conn.close()
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            if row:
                ...
                if total > 0:
                    ...
                    result["consistency"] = min(
                        round(float(row["consistency"]), 1), 100.0
                    )
        ...
        try:
            ...
            if total > 0:
                ...
                result["completeness"] = (
                    round((meta_count / total) * 100, 1) if total > 0 else 100.0
                )
                ...
                result["consistency"] = min(
                    round((distinct_ops / total) * 100, 1) if total > 0 else 100.0,
                    100.0,
                )
                ...
                if last_row and last_row["created_at"]:
                    ...
                    try:
                        ...
                        secs = (
                            (now - last_dt.replace(tzinfo=UTC)).total_seconds()
                            if last_dt.tzinfo
                            else (now - last_dt.replace(tzinfo=UTC)).total_seconds()
                        )
                        if secs < 3600:
                            result["freshness"] = 100.0
                        elif secs < 86400:
