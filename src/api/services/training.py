...
class TrainingService:
        ...
        if USE_POSTGRES:
            ...
            if pool:
                rows = await pool.fetch(
                    "SELECT * FROM training_courses WHERE tenant_id = $1 ORDER BY created_at DESC",
                    tenant_id,
                )
            ...
            try:
                rows = conn.execute(
                    "SELECT * FROM training_courses WHERE tenant_id = ? ORDER BY created_at DESC",
                    (tenant_id,),
                ).fetchall()
                return [dict(r) for r in rows]
            finally:
    ...
    async def create_course(
        self, tenant_id, title, description, modules, duration_hours
    ):
        ...
        if USE_POSTGRES:
            ...
            if pool:
                ...
                row = await pool.fetchrow(
                    "SELECT * FROM training_courses WHERE id = $1", course_id
                )
            ...
            try:
                ...
                conn.commit()
                row = conn.execute(
                    "SELECT * FROM training_courses WHERE id = ?", (course_id,)
                ).fetchone()
                return dict(row) if row else None
            finally:
                conn.close()
    ...
    async def enroll_agent(self, tenant_id, agent_id, course_id):
        ...
        if USE_POSTGRES:
            ...
            if pool:
                ...
                row = await pool.fetchrow(
                    "SELECT * FROM training_enrollments WHERE id = $1", enrollment_id
                )
                return dict(row) if row else None
        else:
            conn = _get_sqlite_conn()
            try:
                ...
                conn.commit()
                row = conn.execute(
                    "SELECT * FROM training_enrollments WHERE id = ?", (enrollment_id,)
                ).fetchone()
                return dict(row) if row else None
            finally:
                conn.close()
    ...
    async def create_coaching_session(
        self, tenant_id, agent_id, coach_id, focus_area, notes
    ):
        ...
        if USE_POSTGRES:
            ...
            if pool:
                ...
                row = await pool.fetchrow(
                    "SELECT * FROM coaching_sessions WHERE id = $1", session_id
                )
                return dict(row) if row else None
        else:
            conn = _get_sqlite_conn()
    ...
    async def list_coaching_sessions(self, tenant_id, agent_id=None):
        if USE_POSTGRES:
            ...
            if pool:
                if agent_id:
                    rows = await pool.fetch(
                        "SELECT * FROM coaching_sessions WHERE tenant_id = $1 AND agent_id = $2 ORDER BY created_at DESC",
                        tenant_id,
                        agent_id,
                    )
                else:
                    rows = await pool.fetch(
                        "SELECT * FROM coaching_sessions WHERE tenant_id = $1 ORDER BY created_at DESC",
                        tenant_id,
                    )
                return [dict(r) for r in rows]
        else:
            ...
            try:
                if agent_id:
                    rows = conn.execute(
                        "SELECT * FROM coaching_sessions WHERE tenant_id = ? AND agent_id = ? ORDER BY created_at DESC",
                        (tenant_id, agent_id),
                    ).fetchall()
                else:
                    rows = conn.execute(
                        "SELECT * FROM coaching_sessions WHERE tenant_id = ? ORDER BY created_at DESC",
                        (tenant_id,),
                    ).fetchall()
                return [dict(r) for r in rows]
            finally:
