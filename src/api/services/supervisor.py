...
class SupervisorService:
    async def get_wallboard_data(self, tenant_id):
        ...
        active_calls = [
            c
            for c in calls
            if c.get("call_status") in ("active", "ringing", "initiated")
        ]
        ...
        online = [
            a
            for a in agents
            if a.get("status") in ("available", "online", "busy", "on_call")
        ]
        ...
        return {
            ...
            "avg_wait_seconds": round(sum(wait_times) / len(wait_times), 1)
            if wait_times
            else 0,
            "longest_wait_seconds": max(wait_times) if wait_times else 0,
            "avg_call_duration_seconds": round(sum(durations) / len(durations), 1)
            if durations
            else 0,
        }
            ...
            team.append(
                {
                    ...
                    "avg_aht": round(
                        a.get("total_talk_time_seconds", 0)
                        / max(a.get("total_calls", 0), 1),
                        1,
                    ),
    ...
    async def get_recent_alerts(self, tenant_id):
        ...
        if (
            wallboard["agents_online"] < wallboard["agents_total"] * 0.5
            and wallboard["agents_total"] > 0
        ):
            alerts.append(
                {
                    "type": "agent_offline",
