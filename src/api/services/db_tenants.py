...
async def create_tenant(
    name, email, slug, phone=None, plan_id=None, settings=None, gdpr_consent=False
):
        ...
        try:
            ...
            """,
                (
                    ...
                    json.dumps(settings or {}),
                    gdpr_consent,
                    now.isoformat() if gdpr_consent else None,
                    api_key,
                    now.isoformat(),
                    now.isoformat(),
                ),
    ...
    if USE_POSTGRES:
        ...
        if pool:
            return await pool.fetchrow(
                "SELECT id, name FROM tenants WHERE api_key = $1", api_key
            )
    else:
        conn = _get_sqlite_conn()
        row = conn.execute(
            "SELECT id, name FROM tenants WHERE api_key = ?", (api_key,)
        ).fetchone()
        conn.close()
        return row
    return None
    ...
    if USE_POSTGRES:
        ...
        if pool:
            row = await pool.fetchrow(
                "SELECT 1 FROM tenants WHERE id = $1 AND api_key = $2",
                tenant_id,
                api_key,
            )
        ...
        conn = _get_sqlite_conn()
        row = conn.execute(
            "SELECT 1 FROM tenants WHERE id = ? AND api_key = ?", (tenant_id, api_key)
        ).fetchone()
        conn.close()
        return row is not None
    return False
...
async def list_agents(tenant_id):
    if USE_POSTGRES:
        ...
        if pool:
            return await pool.fetch(
                "SELECT * FROM agents WHERE tenant_id = $1 ORDER BY name", tenant_id
            )
    else:
        conn = _get_sqlite_conn()
        rows = conn.execute(
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            return (
                json.loads(result)
                if result
                else {"success": False, "error": "function returned null"}
            )
    else:
        conn = _get_sqlite_conn()
        ...
        agent_row = conn.execute(
            "SELECT tenant_id, status FROM agents WHERE id = ?", (agent_id,)
        ).fetchone()
        if agent_row:
            conn.execute(
                """
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            if agent_type is not None:
                fields.append(f"agent_type = ${idx}")
                values.append(agent_type)
                idx += 1
            if skills is not None:
                fields.append(f"skills = ${idx}")
                values.append(json.dumps(skills))
                idx += 1
            if config is not None:
                fields.append(f"config = ${idx}")
        ...
        if fields:
            ...
            values.extend([agent_id, tenant_id])
            conn.execute(
                f"UPDATE agents SET {', '.join(fields)} WHERE id = ? AND tenant_id = ?",
                values,
            )  # nosec B608 — field names are code constants
            conn.commit()
        row = conn.execute("SELECT * FROM agents WHERE id = ?", (agent_id,)).fetchone()
        conn.close()
        return row
...
async def delete_agent_db(agent_id, tenant_id):
    if USE_POSTGRES:
        ...
        conn = _get_sqlite_conn()
        conn.execute(
            "DELETE FROM agents WHERE id = ? AND tenant_id = ?", (agent_id, tenant_id)
        )
...
async def get_available_agents(tenant_id, skills=None):
    ...
    if USE_POSTGRES:
        ...
        conn = _get_sqlite_conn()
        rows = conn.execute(
            "SELECT * FROM agents WHERE tenant_id = ? AND status = 'available' ORDER BY total_calls ASC",
            (tenant_id,),
        ).fetchall()
        conn.close()
        if skills_filter:
            return [
                r
                for r in rows
                if any(s in _parse_skills(r.get("skills")) for s in skills_filter)
            ]
        return rows
...
async def get_user_by_email_db(email):
    if USE_POSTGRES:
        ...
        if pool:
            return await pool.fetchrow(
                "SELECT id, tenant_id, email, password_hash, role, full_name FROM users WHERE email = $1",
                email,
            )
    else:
        conn = _get_sqlite_conn()
...
async def verify_user_email_db(verification_token: str):
    ...
    if USE_POSTGRES:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT id FROM users WHERE verification_token = $1", verification_token
            )
            if row:
                await conn.execute(
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id FROM users WHERE verification_token = ?", (verification_token,)
        )
        row = cursor.fetchone()
        if row:
            cursor.execute(
...
async def update_tenant_subscription_db(
    tenant_id: str,
    ...
    if USE_POSTGRES:
        ...
        async with pool.acquire() as conn:
            return await conn.fetchrow(
                "SELECT id, plan_id FROM tenants WHERE stripe_customer_id = $1",
                stripe_customer_id,
            )
    else:
        conn = _get_sqlite_conn()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, plan_id FROM tenants WHERE stripe_customer_id = ?",
            (stripe_customer_id,),
        )
        row = cursor.fetchone()
        conn.close()
...
async def record_usage_db(
    tenant_id: str, metric: str, quantity: float, period_start: str, period_end: str
):
    ...
    if USE_POSTGRES:
        ...
        async with pool.acquire() as conn:
            ...
            if industry:
                ...
                idx += 1
            query += (
                f" ORDER BY score DESC, created_at DESC LIMIT ${idx} OFFSET ${idx + 1}"
            )
    ...
    if USE_POSTGRES:
        ...
        async with pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM leads WHERE id = $1 AND tenant_id = $2", lead_id, tenant_id
            )
            return result != "DELETE 0"
    else:
        conn = _get_sqlite_conn()
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM leads WHERE id = ? AND tenant_id = ?", (lead_id, tenant_id)
        )
...
async def bulk_update_leads_db(
    tenant_id: str, lead_ids: list[str], updates: dict
) -> int:
...
async def create_script_db(
    tenant_id: str, name: str, content: dict, variables: list[dict] = None
) -> dict:
    """Create a new script."""
...
async def get_script_db(script_id: str, tenant_id: str) -> dict | None:
    """Get a script by ID."""
    if USE_POSTGRES:
        pool = await get_pg_pool()
        async with pool.acquire() as conn:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM scripts WHERE id = ? AND tenant_id = ?",
            (script_id, tenant_id),
        )
        row = cursor.fetchone()
        conn.close()
    ...
    if row and isinstance(row, dict):
        ...
        row_dict["variables"] = (
            json.loads(row_dict["variables"])
            if isinstance(row_dict["variables"], str)
            else row_dict["variables"]
        )
        return row_dict
...
async def list_scripts_db(
    tenant_id: str, is_active: bool = None, limit: int = 100, offset: int = 0
) -> list[dict]:
...
async def update_script_db(
    script_id: str, tenant_id: str, updates: dict
) -> dict | None:
...
async def delete_script_db(script_id: str, tenant_id: str) -> bool:
    ...
    if USE_POSTGRES:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM scripts WHERE id = ? AND tenant_id = ?", (script_id, tenant_id)
        )
        affected = cursor.rowcount
        conn.commit()
        conn.close()
...
async def get_script_template_db(template_id: str) -> dict | None:
    """Get a script template by ID."""
    if USE_POSTGRES:
        pool = await get_pg_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM script_templates WHERE id = $1", template_id
            )
    else:
        conn = _get_sqlite_conn()
        cursor = conn.cursor()
...
async def list_script_templates_db(
    industry: str = None, limit: int = 100, offset: int = 0
) -> list[dict]:
...
async def create_script_template_db(
    name: str,
    description: str,
    industry: str,
    content: dict,
...
async def delete_script_template_db(template_id: str) -> bool:
    ...
    if USE_POSTGRES:
        ...
        async with pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM script_templates WHERE id = $1", template_id
            )
            return result != "DELETE 0"
    else:
        conn = _get_sqlite_conn()
