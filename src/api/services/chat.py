...
class ChatService:
    ...
    async def create_session(
        self, visitor_id, tenant_id, name=None, email=None, initial_message=None
    ):
        ...
        _in_memory_sessions[session["id"]] = {
        ...
        logger.info(
            "chat_session_created", session_id=session["id"], visitor_id=visitor_id
        )
        return session
    ...
    async def assign_agent(self, session_id, agent_id):
        ...
        result = await update_chat_session_db(
            session_id, agent_id=agent_id, status="active", assigned_at=now
        )
    ...
    async def close_session(self, session_id):
        ...
        now = datetime.now(UTC).isoformat()
        result = await update_chat_session_db(
            session_id, status="closed", closed_at=now
        )
