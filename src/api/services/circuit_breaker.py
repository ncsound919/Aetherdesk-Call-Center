...
class CircuitBreaker:
        ...
        if self._state == CircuitState.HALF_OPEN:
            if self._half_open_calls >= self.half_open_max_calls:
                raise CircuitBreakerOpenError(
                    f"Circuit breaker '{self.name}' is HALF_OPEN and at max probe calls"
                )
    ...
    async def _transition_to(self, new_state: CircuitState):
        ...
        try:
            await log_circuit_breaker_event_db(
                self.name, old_state.value, new_state.value, self._failure_count
            )
        except Exception as e:
            logger.warning(
                "circuit_breaker_event_log_failed", name=self.name, error=str(e)
            )
    ...
    async def reset(self):
        ...
        try:
            await log_circuit_breaker_event_db(
                self.name, old_state.value, "RESET", self._failure_count
            )
        except Exception as e:
            logger.warning(
                "circuit_breaker_event_log_failed", name=self.name, error=str(e)
