...
_POSTGRES_TYPE_MAP = [
    (
        re.compile(
            r"\bUUID\s+PRIMARY\s+KEY\s+DEFAULT\s+uuid_generate_v4\(\)", re.IGNORECASE
        ),
        "TEXT PRIMARY KEY",
    ),
    ...
    (
        re.compile(r"\bBOOLEAN\s+NOT\s+NULL\s+DEFAULT\s+(\S+)", re.IGNORECASE),
...
_STRIP_LINES = [
    ...
    re.compile(
        r"ALTER\s+TABLE\s+\w+\s+DISABLE\s+ROW\s+LEVEL\s+SECURITY", re.IGNORECASE
    ),
    re.compile(
        r"ALTER\s+TABLE\s+\w+\s+(NO\s+)?FORCE\s+ROW\s+LEVEL\s+SECURITY", re.IGNORECASE
    ),
