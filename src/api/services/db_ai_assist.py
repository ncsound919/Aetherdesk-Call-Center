...
async def create_knowledge_snippet_db(
    tenant_id, title, content, tags=None, category="general"
):
    ...
    if USE_POSTGRES:
        ...
        if pool:
            ...
            row = await pool.fetchrow(
                "SELECT * FROM knowledge_snippets WHERE id = $1", snippet_id
            )
        ...
        try:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT * FROM knowledge_snippets WHERE id = ?", (snippet_id,)
            ).fetchone()
            return dict(row) if row else None
        finally:
            conn.close()
...
async def delete_knowledge_snippet_db(tenant_id, snippet_id):
    if USE_POSTGRES:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM knowledge_snippets WHERE id = ? AND tenant_id = ?",
