...
# Agent cache TTL: how long (seconds) a cached TenantAgent is reused before
# being recreated. Keeps agent prompts / tool lists fresh after DB config changes.
...
class ReActAgent:
    ...
    async def step(self, history: list[dict[str, str]], user_input: str, tenant_id: str) -> AgentResponse:
        ...
        try:
            # Langfuse trace for this agent step
            if lf:
                trace = lf.trace(
                    name=f"agent_step:{self.name}",
                    metadata={"tenant_id": tenant_id, "agent_name": self.name},
                    tags=[tenant_id, self.name],
                )
                trace_id = trace.id
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                for attempt in range(2):  # Self-healing retry loop
                    try:
                        ...
                        if generation:
                            try:
                                generation.update(output=ai_msg[:500])
                            except Exception:
                                pass
                        ...
                        if "response" in parsed:
                            ...
                            if ao_session:
                                try:
                                    ...
                                    pass
                                ao_session = None  # Prevent double end in finally
                            # Langfuse scoring
                            if trace_id:
                                score_call(
                                    call_sid=trace_id,
                                    ...
                                    value=latency,
                                    comment=f"Agent {self.name} responded in {latency:.0f}ms",
                                )
...
class Orchestrator:
    ...
    async def get_agent(self, tenant_id: str, profile_id: str) -> TenantAgent:
        ...
        # Create (or recreate after TTL expiry)
        if key in self.agents:
            logger.info("agent_cache_evicted", key=key, age_s=round(now - cached_ts, 1))
        agent = TenantAgent(tenant_id, profile_id, self.actions)
        self.agents[key] = agent
        self._agent_timestamps[key] = now
        return agent

    async def start_cleanup_loop(self):
        """Background coroutine: evict stale agent cache entries every TTL interval."""
        while True:
            await asyncio.sleep(AGENT_CACHE_TTL_SECONDS)
            now = time.time()
            cutoff = now - AGENT_CACHE_TTL_SECONDS
            stale_keys = [
                k for k, ts in list(self._agent_timestamps.items()) if ts < cutoff
            ]
            for k in stale_keys:
                self.agents.pop(k, None)
                self._agent_timestamps.pop(k, None)
            if stale_keys:
