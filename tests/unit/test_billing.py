...
class TestCheckout:
    ...
    async def test_checkout_creates_session(self, auth_bearer):
        from api.routers.billing import create_checkout, CheckoutRequest

        with patch("api.routers.billing.get_price_id", return_value="price_test"), \
             patch("api.routers.billing.create_checkout_session", new_callable=AsyncMock) as mock_session, \
    ...
    async def test_checkout_rejects_unknown_plan(self, auth_bearer):
        ...
        with patch("api.routers.billing.get_price_id", return_value=None), \
             patch("api.routers.billing.get_tenant_db", new_callable=AsyncMock):
...
class TestPortal:
    ...
    async def test_portal_requires_stripe_customer(self, auth_bearer):
        ...
        with patch("api.routers.billing.get_tenant_db", new_callable=AsyncMock) as mock_get_tenant:
...
class TestSubscription:
    ...
    async def test_subscription_returns_plan(self, auth_bearer):
        ...
        with patch("api.routers.billing.get_tenant_db", new_callable=AsyncMock) as mock_get_tenant, \
             patch("api.routers.billing.get_tenant_plan_db", new_callable=AsyncMock) as mock_get_plan:
...
class TestUsage:
    ...
    async def test_usage_recorded(self, auth_bearer):
        from api.routers.billing import report_usage, UsageRequest
...
class TestGetBilling:
    def test_get_billing_returns_summary(self, client):
        ...
        with patch("api.routers.billing.get_billing_summary", new_callable=AsyncMock) as mock_get:
