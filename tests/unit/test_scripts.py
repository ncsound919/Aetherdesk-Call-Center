...
class TestScriptCRUD:
    ...
    async def test_create_script_success(self, auth_bearer):
        from apps.api.routers.scripts import create_script, ScriptCreate

        with patch("apps.api.services.db_tenants.create_script_db", new_callable=AsyncMock) as mock_create:
    ...
    async def test_list_scripts_with_filter(self, auth_bearer):
        from apps.api.routers.scripts import list_scripts

        with patch("apps.api.services.db_tenants.list_scripts_db", new_callable=AsyncMock) as mock_list:
    ...
    async def test_get_script_success(self, auth_bearer):
        from apps.api.routers.scripts import get_script

        with patch("apps.api.services.db_tenants.get_script_db", new_callable=AsyncMock) as mock_get:
    ...
    async def test_update_script_increments_version(self, auth_bearer):
        from apps.api.routers.scripts import update_script, ScriptUpdate

        with patch("apps.api.services.db_tenants.update_script_db", new_callable=AsyncMock) as mock_update:
    ...
    async def test_delete_script(self, auth_bearer):
        from apps.api.routers.scripts import delete_script

        with patch("apps.api.services.db_tenants.delete_script_db", new_callable=AsyncMock) as mock_delete:
...
class TestTemplateOperations:
    ...
    async def test_list_templates(self, auth_bearer):
        from apps.api.routers.scripts import list_templates

        with patch("apps.api.services.db_tenants.list_script_templates_db", new_callable=AsyncMock) as mock_list:
            mock_list.return_value = [
    ...
    async def test_get_template(self, auth_bearer):
        from apps.api.routers.scripts import get_template

        with patch("apps.api.services.db_tenants.get_script_template_db", new_callable=AsyncMock) as mock_get:
    ...
    async def test_clone_template(self, auth_bearer):
        from apps.api.routers.scripts import clone_template

        with patch("apps.api.services.db_tenants.get_script_template_db", new_callable=AsyncMock) as mock_get, \
             patch("apps.api.services.db_tenants.create_script_db", new_callable=AsyncMock) as mock_create:
