...
# Helper to get the underlying async function from StructuredTool
def get_tool_func(tool):
    # StructuredTool stores async functions in .coroutine
...
class TestLookupInvoiceTool:
    ...
    async def test_invoice_found(self, mock_actions):
        ...
        # Call the underlying async function via .func
        result = await get_tool_func(orchestrator.lookup_invoice)(
            invoice_id="INV-123",
            tenant_id="tenant-1",
...
class TestGetOrderStatusTool:
    ...
    async def test_order_found(self, mock_actions):
        ...
        result = await get_tool_func(orchestrator.get_order_status)(
            order_id="ORD-456",
            tenant_id="tenant-1",
            actions_instance=mock_actions
        )
...
class TestSearchKnowledgeBaseTool:
    ...
    async def test_knowledge_results(self, mock_actions):
        with patch("apps.api.services.rag.rag_service") as mock_rag:
            ...
            result = await get_tool_func(orchestrator.search_knowledge_base)(
                query="refund policy",
                tenant_id="tenant-1"
            )
...
class TestHandoffToHumanTool:
    ...
    async def test_handoff_initiated(self, mock_actions):
        ...
        result = await get_tool_func(orchestrator.handoff_to_human)(
...
class TestEscalateToSupervisorTool:
    ...
    async def test_escalation_returns_message(self, mock_actions):
        result = await get_tool_func(orchestrator.escalate_to_supervisor)(
